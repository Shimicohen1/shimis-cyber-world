...
def generate_yaml(categorized, drive_ids, known_dates):
    """Generate the resources.yml content with added_date tracking."""
    ...
    for cat_name, icon in CAT_ORDER:
        ...
        for subcat_name in sorted(categorized[cat_name].keys()):
            ...
            for item in sorted(categorized[cat_name][subcat_name], key=lambda x: x['title'].lower()):
                fname = item['file']
                added = known_dates.get(fname, today)
                lines.append(f'          - title: {yaml_escape(item["title"])}')
                lines.append(f'            file: {yaml_escape(fname)}')
                lines.append(f'            type: {item["type"]}')
                lines.append(f'            added: "{added}"')
                if fname in drive_ids:
                    did = drive_ids[fname]
                    lines.append(f'            url: https://drive.google.com/file/d/{did}/view')
                if added == today and fname not in known_dates:
                    new_resources.append({
                        'title': item['title'],
                        'file': fname,
                        'type': item['type'],
                        'category': cat_name,
                        'url': f'https://drive.google.com/file/d/{drive_ids[fname]}/view' if fname in drive_ids else None
                    })
...
def load_known_dates():
    """Load added_date from existing resources.yml so we preserve them."""
    dates = {}
    if not os.path.exists(RESOURCES_YML):
        return dates
    with open(RESOURCES_YML, 'r') as f:
    ...
    for cat in data['categories']:
        for sub in cat.get('subcategories', []):
            for res in sub.get('resources', []):
                if 'file' in res and 'added' in res:
                    dates[res['file']] = res['added']
    return dates


def generate_post(new_resources):
    """Auto-generate a Jekyll post for new Vault additions."""
    today = datetime.date.today()
    slug = f"{today.isoformat()}-vault-update"
    post_path = os.path.join(REPO_DIR, '_posts', f'{slug}.md')
    ...
    cats_str = ', '.join(sorted(by_cat.keys())[:4])
    if cat_count > 4:
        cats_str += f' + {cat_count - 4} more'

    lines = [
        '---',
        'layout: post',
        f'title: "Vault Update — {count} New Resources Added"',
        f'date: {today.isoformat()}',
        'tags: [vault-update, resources]',
        f'description: "{count} new resources across {cat_count} categories: {cats_str}"',
        '---',
        '',
        f'**{count} new resources** just landed in the [Vault](/library/).',
        '',
    ]
    ...
    lines.append('Browse the full collection in the [Resource Vault](/library/).')
...
def git_commit_push(has_new_post=False):
...
def main():
    ...
    # 6. Load known dates from existing YAML
    known_dates = load_known_dates()
    log(f"Known resource dates loaded: {len(known_dates)}")

    # 7. Generate YAML with dates + detect new resources
    yaml_content, new_resources = generate_yaml(categorized, drive_ids, known_dates)
    with open(RESOURCES_YML, 'w') as f:
        f.write(yaml_content)
    log(f"Written: {RESOURCES_YML}")
    log(f"New resources detected: {len(new_resources)}")

    # 8. Auto-generate post for new resources
    has_new_post = False
    if new_resources:
        post_path = generate_post(new_resources)
        log(f"Generated vault update post: {post_path}")
        has_new_post = True

    # 9. Commit & push
    pushed = git_commit_push(has_new_post)
    if pushed:
